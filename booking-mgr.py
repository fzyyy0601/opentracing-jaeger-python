...
def check_cinema(movie):
    ...
    with tracer.start_span('CheckCinema', child_of=get_current_span()) as span:
        with span_in_context(span):
            ...
            time.sleep(num)
            cinema_details = "Cinema Details "+movie
            flags = ['false', 'true', 'false']
            random_flag = random.choice(flags)
            span.set_tag('error', random_flag) # using tags
            span.log_kv({'event': 'CheckCinema' , 'value': cinema_details }) # using logs
